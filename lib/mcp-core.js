...
async function handleTool(name, args) {
  switch (name) {
    ...
    case 'myl_crm_intelligence': {
      ...
      const warm  = calls.filter(c => c.enrichment?.lead_quality === 'Warm').length;
      const cold  = calls.filter(c => c.enrichment?.lead_quality === 'Cold').length;
      const scores = calls.map(c => c.enrichment?.lead_score).filter(n => typeof n === 'number');
      const avg_score = scores.length ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;

      const byCat = {};
      calls.forEach(c => {
        const cat = c.enrichment?.garment_category || 'unknown';
        byCat[cat] = (byCat[cat] || 0) + 1;
      });

      const byStage = {};
      calls.forEach(c => {
        const stage = c.enrichment?.founder_stage || 'unknown';
      ...
      return {
        ...
        conversion_funnel: {
          design_ready:              calls.filter(c => c.enrichment?.design_readiness !== 'NONE' && c.enrichment?.design_readiness).length,
          high_validation_appetite:  calls.filter(c => c.enrichment?.validation_appetite === 'HIGH').length,
          recommended_pro_tier:      calls.filter(c => (c.enrichment?.lead_score || 0) >= 70).length,
        },
