...
// Called by Railway Cron every 2 minutes → self-healing alternative to Zoho webhook.
...
router.post('/leads', async (req, res) => {
  ...
  if (process.env.SANDBOX_MODE === 'true') {
    return res.json({ sandbox: true, skipped: true });
  }

  try {
    const token = await getZohoToken();

    // Fetch leads created in the last 10 minutes with no AI call status
